...
//
// The panel body owns the flow and is mounted only while the accordion is open, so closing it resets
// to idle. The old inline version kept a half-armed red "Delete permanently" button across a close
// and reopen; requiring the first tap again is the safe direction for the one action with a
// deadline on its undo.
...
export default function DangerSection({ album, open, onToggle }: Props) {
  const { t } = useT()
  return (
    <section style={{ ...settingsSectionStyle, marginBottom: 0 }}>
      ...
      {open && <DangerPanel album={album} />}
    </section>
  )
}

function DangerPanel({ album }: { album: Album }) {
  const { t } = useT()
        ...
        {flow.phase === 'deleted' ? (
          ...
          <div>
            <p className="text-xs leading-relaxed mb-3" style={{ color: '#7A2A1F' }}>
              {t('ot.albumDeleted')} {t('ot.restorableFor').replace('{days}', String(flow.restorableForDays))}
            </p>
