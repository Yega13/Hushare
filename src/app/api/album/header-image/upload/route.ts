...
const MAX_HEADER_IMAGE_BYTES = 20 * 1024 * 1024  // 20 MB

// Presign for a custom header/hero photo — mirrors /api/album/background/upload exactly, just a
// different R2 prefix and target field (header_image instead of background_theme).
export async function POST(req: Request) {
  ...
  if (typeof fileSize !== 'number' || !Number.isFinite(fileSize) || !Number.isInteger(fileSize) || fileSize <= 0 || fileSize > MAX_HEADER_IMAGE_BYTES) {
    return NextResponse.json({ error: `Invalid file size (max ${MAX_HEADER_IMAGE_BYTES / 1024 / 1024} MB)` }, { status: 400, headers: NO_STORE })
  }
  ...
  // failOpen:false — deny on outage rather than allow unbounded header-image uploads.
  const albumRl = await checkRateLimit(
    clientIpKey(req, `header_upload:${access.album.id}`),
    600, 5, { failOpen: false },
  )
  if (!albumRl.ok) {
    return NextResponse.json(
      { error: 'Too many header photo uploads. Try again later.' },
  ...
  const key = `headers/${access.album.id}/${uuid()}.${ext}`
  ...
  try {
    ...
    console.error('[album/header-image/upload] presign failed:', e instanceof Error ? e.message : String(e))
