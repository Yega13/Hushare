...
// no longer compiles at the call sites (that is the point of the brand). A test is the one place
// that legitimately manufactures readings to drive the logic, and it says so with this helper
// rather than with twenty anonymous casts.
const at = (n: number) => n as Millis
...
describe('a broadcast that is really my own edit coming back', () => {
  it('is not acted on immediately', () => {
    ...
    const a = onSettingsBroadcast({ designerOpen: false, now: at(NOW), lastLocalEditAt: at(NOW - 100), quietMs: QUIET })
  ...
  it('pushes the wait out as the owner keeps editing', () => {
    ...
    const early = onSettingsBroadcast({ designerOpen: false, now: at(NOW), lastLocalEditAt: at(NOW - 2400), quietMs: QUIET })
    const late = onSettingsBroadcast({ designerOpen: false, now: at(NOW), lastLocalEditAt: at(NOW - 10), quietMs: QUIET })
...
describe('a broadcast that is genuine news', () => {
  it('is fetched straight away', () => {
    expect(onSettingsBroadcast({ designerOpen: false, now: at(NOW), lastLocalEditAt: at(NOW - QUIET), quietMs: QUIET }).kind).toBe('refetch')
    expect(onSettingsBroadcast({ designerOpen: false, now: at(NOW), lastLocalEditAt: at(0), quietMs: QUIET }).kind).toBe('refetch')
  })
  ...
  it('holds at the edge of the quiet window', () => {
    ...
    expect(onSettingsBroadcast({ designerOpen: false, now: at(NOW), lastLocalEditAt: at(NOW - QUIET), quietMs: QUIET }).kind).toBe('refetch')
    expect(onSettingsBroadcast({ designerOpen: false, now: at(NOW), lastLocalEditAt: at(NOW - QUIET + 1), quietMs: QUIET }).kind).toBe('schedule')
  })
})
...
describe('the Album Designer is never overwritten while it is open', () => {
  it('owes the refetch instead of doing it, whatever else is true', () => {
    ...
    for (const lastLocalEditAt of [at(0), at(NOW - 10), at(NOW - 999_999)]) {
      expect(onSettingsBroadcast({ designerOpen: true, now: at(NOW), lastLocalEditAt, quietMs: QUIET }).kind)
        .toBe('owe')
    }
  })
})
...
describe('a response that arrived too late is not applied', () => {
  ...
  const base = { disposed: false, requestStartedAt: at(NOW), lastLocalEditAt: at(NOW - 5000) }
  ...
  it('drops a response overtaken by the owner\'s own edit', () => {
    ...
    expect(shouldCommitSettings(DATA, { ...base, lastLocalEditAt: at(NOW + 1) })).toBe(false)
  })
  ...
  it('commits when the edit and the request are simultaneous', () => {
    ...
    expect(shouldCommitSettings(DATA, { ...base, lastLocalEditAt: at(NOW) })).toBe(true)
  })
...
describe('a backwards clock cannot postpone the refresh forever', () => {
  it('never waits longer than the quiet window, whatever the clock says', () => {
    for (const jumpMs of [1, 5_000, 3_600_000, 86_400_000]) {
      const a = onSettingsBroadcast({ designerOpen: false, now: at(NOW), lastLocalEditAt: at(NOW + jumpMs), quietMs: QUIET })
      if (a.kind !== 'schedule') throw new Error('expected schedule')
...
describe('a dragged slider produces one refetch, not one per broadcast', () => {
  ...
  it('collapses a burst of broadcasts into a single trailing fetch', () => {
    ...
    for (let i = 0; i < 10; i++) sync.onBroadcast({ designerOpen: false, now: at(NOW + i), lastLocalEditAt: at(NOW + i) })
  ...
  it('owes rather than queues while the Designer is open', () => {
    ...
    sync.onBroadcast({ designerOpen: true, now: at(NOW), lastLocalEditAt: at(NOW) })
    expect(owed).toBe(1)
