...
import { RECONCILE_COPY_BUDGET, RECONCILE_TIME_BUDGET_MS, type BackupBucket } from '@/lib/server/media-backup'

// THE EVERY-MINUTE WALK UNDER THE PHOTO BACKUP.
//
// The lib's tests prove the walk. These prove the route drives it: from the saved position, with the
// real budgets, through FixedLengthStream, saving where it got to -- and that each way it can fail
// reaches the error panel, since nothing reads this route's response. The database, the Cloudflare
// context and the error reporter are mocked at the module boundary; the walk and the prune are real.
...
function bucket(seed: Record<string, Obj>, opts: { locked?: boolean; failPut?: (k: string) => boolean; failHead?: (k: string) => boolean; onGet?: () => void } = {}) {
...
describe('the backup walk route', () => {
  ...
  it('between passes it reads the position and stops -- no listing, no write', async () => {
    cfg.stateValue = JSON.stringify({ lastPassCompletedAt: new Date(Date.now() - 3_600_000).toISOString() })
    const source = bucket({ 'albums/a/p.jpg': old() })
    rig(source, bucket({}, { locked: true }))
    const res = await POST(post())
  ...
  it('mid-pass it carries on from the SAVED position', async () => {
    cfg.stateValue = JSON.stringify({ startAfter: 'albums/b', passStartedAt: new Date().toISOString() })
    const backup = bucket({}, { locked: true })
  ...
  it('copies at most 200 in a run, saves where it stopped, and does not prune mid-pass', async () => {
    const seed: Record<string, Obj> = {}
    for (let i = 0; i <= 200; i++) seed[`albums/k${String(i).padStart(3, '0')}`] = old()
    const backup = bucket({ 'albums/zz-gone': old(), '_deleted/albums/zz-gone': { body: '', uploaded: new Date(Date.now() - 40 * DAY) } }, { locked: true })
    rig(bucket(seed), backup)
    const res = await POST(post())
    expect(RECONCILE_COPY_BUDGET).toBe(200)
    expect(await res.json()).toMatchObject({ copied: 200, passComplete: false, pruned: null })
    expect(savedState().startAfter).toBe('albums/k199')
    expect(backup.store.has('albums/zz-gone')).toBe(true)
  })
  ...
  it('a finished pass prunes deletions past the grace period', async () => {
    const backup = bucket({
      'albums/gone.jpg': old(),
      '_deleted/albums/gone.jpg': { body: '', uploaded: new Date(Date.now() - 40 * DAY) },
    }, { locked: true })
    rig(bucket({}), backup)
    const res = await POST(post())
    expect(await res.json()).toMatchObject({ passComplete: true, pruned: { removed: 1, kept: 0, failed: 0 } })
