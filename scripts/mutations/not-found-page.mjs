...
export default {
  ...
  mutations: [
    ...
    { name: 'the pile has no height -- the prints spill over the logo and heading',
      from: 'height: PILE_HEIGHT,', to: 'height: 0,' },
    { name: 'the pile loses its narrow-screen class -- on a 320px phone the side prints are cut off at the edge',
      from: ' className="hush-404-pile"', to: '' },
    { name: 'the drift lift is typed out again -- the geometry the tests check stops being the page',
      from: '${DRIFT.lift}px', to: '7px' },
    { name: 'no narrow-screen rules at all -- the pile is full size on every phone',
      from: "${NARROW.map((n) => `@media (max-width: ${n.maxWidth}px) { .hush-404-pile { transform: scale(${n.scale}); } }`).join(' ')}", to: '' },
