...
export default {
  ...
  { name: 'locked without a reveal time is a reveal gate',
    from: "  if (json.locked === true && json.reveal_at) {", to: "  if (json.locked === true) {" },
  { name: 'a non-string reveal time is rendered as the album (the drift a reviewer caught)',
    from: "    if (typeof json.reveal_at !== 'string') return { kind: 'error' }\n", to: "" },
  { name: 'a truthy locked counts',
    from: "  if (json.locked === true && json.reveal_at) {", to: "  if (json.locked && json.reveal_at) {" },
  { name: 'a truthy password_required counts',
    from: "  if (json.password_required === true) {", to: "  if (json.password_required) {" },
