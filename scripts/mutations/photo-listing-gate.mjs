...
export default {
  ...
  { name: 'the DELTA read skips the filter, so every refresh leaks what the album page hides',
    from: "      (isOwner ? query : query.eq('hidden', false))\n        .gt('created_at', opts.since)", to: "      query\n        .gt('created_at', opts.since)" },
  { name: "the delta's own count ignores the filter, so the total contradicts the photos beside it",
    from: "      isOwner ? countQ : countQ.eq('hidden', false),", to: "      countQ," },
  { name: "the delta's newest time ignores the filter, so a hidden upload announces itself on every refresh",
    from: "      (isOwner ? newestQ : newestQ.eq('hidden', false))", to: "      newestQ" },
  { name: 'a failed delta read answers with a total of zero instead of unavailable',
    from: "    if (rows.error || counted.error || newest.error) return { kind: 'unavailable' }", to: "    if (rows.error || counted.error || newest.error) return { kind: 'ok', photos: [], total: 0 }" },
